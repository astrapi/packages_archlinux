...
while IFS= read -r line; do
  ...
  # Manche Pakete pruefen ihren Download nicht ueber source=()/sha256sums(),
  # sondern per eigener _sha256-Variable + sha256sum -c in build()/package()
  # (kein echtes makepkg, daher kein automatisches Pruefen eingebaut). Die
  # haengt am Tarball der ALTEN Version -- ohne diesen Schritt waere der
  # naechste Bau am Hash-Check gescheitert (siehe T-022-DEBIAN im
  # packages-Vault, wo dieser Fall zuerst auffiel). Bei den aktuellen
  # Arch-Paketen (sha256sums=('SKIP' ...)) greift das nicht, bleibt aber
  # fuer kuenftige Pakete mit demselben Muster nuetzlich.
